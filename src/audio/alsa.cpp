...
void test_alsa2()
{
    ...
    u32 channel_count = 0;
    snd_pcm_hw_params_get_channels(params, &channel_count);
    printf("channels = %d\n", channel_count);
    ...
    snd_pcm_hw_params_get_buffer_time(params, &val2, &dir);
    printf("buffer time = %d us\n", val2);

    snd_pcm_hw_params_get_buffer_size(params, &frames);
    printf("buffer size = %d frames\n", (u32)frames);

    snd_pcm_hw_params_get_periods(params, &val2, &dir);
    printf("periods per buffer = %d frames\n", val2);

    snd_pcm_hw_params_get_rate_numden(params, &val2, &val2);
    printf("exact rate = %d/%d bps\n", val2, val2);

    val2 = snd_pcm_hw_params_get_sbits(params);
    printf("significant bits = %d\n", val2);

    //    snd_pcm_hw_params_get_tick_time(params, &val2, &dir);
    //    printf("tick time = %d us\n", val2);

    val2 = snd_pcm_hw_params_is_batch(params);
    printf("is batch = %d\n", val2);

    val2 = snd_pcm_hw_params_is_block_transfer(params);
    printf("is block transfer = %d\n", val2);

    val2 = snd_pcm_hw_params_is_double(params);
    printf("is double = %d\n", val2);

    val2 = snd_pcm_hw_params_is_half_duplex(params);
    printf("is half duplex = %d\n", val2);

    val2 = snd_pcm_hw_params_is_joint_duplex(params);
    printf("is joint duplex = %d\n", val2);

    val2 = snd_pcm_hw_params_can_overrange(params);
    printf("can overrange = %d\n", val2);

    val2 = snd_pcm_hw_params_can_mmap_sample_resolution(params);
    printf("can mmap = %d\n", val2);

    val2 = snd_pcm_hw_params_can_pause(params);
    printf("can pause = %d\n", val2);

    val2 = snd_pcm_hw_params_can_resume(params);
    printf("can resume = %d\n", val2);

    val2 = snd_pcm_hw_params_can_sync_start(params);
...
static int
direct_loop(snd_pcm_t* handle, signed short* samples ATTRIBUTE_UNUSED, snd_pcm_channel_area_t* areas ATTRIBUTE_UNUSED)
{
    double                        phase = 0;
    const snd_pcm_channel_area_t* my_areas;
    snd_pcm_uframes_t             offset, frames, size;
...
static int direct_write_loop(snd_pcm_t* handle, signed short* samples, snd_pcm_channel_area_t* areas)
{
    double        phase = 0;
    signed short* ptr;
    int           err, cptr;
