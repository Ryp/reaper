...
pub fn createPhysicalDeviceInfo(
    ...
    info.macro_features = try fillMacroFeatures(vki, handle);
...
fn checkComputeStoresToDepth(vki: anytype, handle: vk.PhysicalDevice) !bool {
    ...
    // FORMAT_NOT_SUPPORTED is the answer "no"; anything else is a real driver
    // failure and must not be silently reported as an unsupported feature.
    vki.getPhysicalDeviceImageFormatProperties2(handle, &format_info, &props) catch |err| switch (err) {
...
fn fillMacroFeatures(vki: anytype, handle: vk.PhysicalDevice) !PhysicalDeviceInfo.MacroFeatures {
    return .{
        .compute_stores_to_depth = try checkComputeStoresToDepth(vki, handle),
    };
}
...
pub fn checkPhysicalDevice(
    ...
    // NOTE: no vendor filtering here — vulkan_check_physical_device() in
    // Backend.cpp accepts any vendor and we match it.
...
fn checkDeviceExtensions(
    ...
    for (required) |req| {
        ...
        if (!found) {
            log.err("device extension '{s}' not supported", .{req_name});
...
pub fn logPhysicalDevice(info: *const PhysicalDeviceInfo) void {
    log.info("selecting device '{s}'", .{std.mem.sliceTo(&info.properties.device_name, 0)});
